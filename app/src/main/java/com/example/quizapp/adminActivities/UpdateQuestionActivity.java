...
import com.example.quizapp.databinding.ActivityUpdateQuestionBinding;
...
public class UpdateQuestionActivity extends BaseAdminActivity {//Checked
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        loadInfo();
        binding.btUpdate.setOnClickListener(v->updateQuestion());
    }

    private void loadInfo() {
        binding.etText.setText(question.getQuestionText());
        binding.etOp1.setText(question.getOptions().get("1"));
        binding.etOp2.setText(question.getOptions().get("2"));
        binding.etOp3.setText(question.getOptions().get("3"));
        binding.etOp4.setText(question.getOptions().get("4"));

        if(question.getCorrectOptions().contains(1)) binding.cbOp1.setChecked(true);
        if(question.getCorrectOptions().contains(2)) binding.cbOp2.setChecked(true);
        if(question.getCorrectOptions().contains(3)) binding.cbOp3.setChecked(true);
        if(question.getCorrectOptions().contains(4)) binding.cbOp4.setChecked(true);
    }
    ...
    private void updateQuestion() {
        ...
        question.setCourse(this.question.getCourse());
        question.setOptions(options);
        question.setCorrectOptions(correctOptions);
        question.setQuestionId(this.question.getQuestionId());

        SplashActivity.dbHelper.updateQuestion(question, new OnModifyListener() {
            @Override
            public void onSuccess() {
                Snackbar.make(binding.getRoot(),"Question updated successfully",Snackbar.LENGTH_LONG).show();
            }
